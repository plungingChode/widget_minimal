...
Lista::Lista(Application *app, int x, int y, int w, int sorok_, vector<string> elemek_)
    : Widget(app, x, y, w, (gout.cascent()+gout.cdescent())*sorok_), 
      sorok(sorok_), elemek(elemek_), kijelolt(-1), legfelso(0)
{
...
{
    legfelso = min(max(legfelso+d, 0), int(elemek.size())-sorok);
}
...
void Lista::handle(genv::event ev)
{
    if (ev.type == ev_mouse)
    {
        ...
        {
            for (int i = 0; i < elem_helyek.size(); i++)
            {
...
void Lista::draw()
{
    ...
    int csuszka_y = _y+legfelso*csuszka_h;
